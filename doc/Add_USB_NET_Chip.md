...
index 7568629bc9..3243d11489 100644
...
+	tristate "Davicom SR9800 based USB 1.1 10/100 ethernet devices"
+	depends on USB_USBNET
+	select CRC32
+	help
+	  This option adds support for Davicom SR9800 based USB 1.1
+	  10/100 Ethernet adapters.
+
...
index f72e866d99..602bfa754b 100644
--- a/kernel/drivers/net/usb/Makefile
+++ b/kernel/drivers/net/usb/Makefile
@@ -36,4 +36,4 @@ rmnet_usb-y			:= rmnet_usb_ctrl.o rmnet_usb_data.o
...
+/*
+ * define the mac stored file address in android,
+ * which should be with read/write capability
+ */
...
+     * see https://github.com/torvalds/linux/commit/eb85569fe2d06c2fbf4de7b66c263ca095b397aa#diff-55f64f37c6ee03fbce14491253b57b43
+     */
...
+}
+
+void sr_recover(struct usbnet *dev)
+{
...
+         printk("usb_reset_device err\n");
+     }
+     if(r < 0)
+     {
+         printk("usb_reset_device err usbnet==null\n");
+         return;
+     }
+     r = usb_reset_device(udev);
+     printk("usb_reset_device return:%d\n", r);
...
+		return ret;
+	}
+	printk("mac addr : 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x\n",
...
+				if (IS_ERR(fp))
+				{
+					printk("file not exist, create it now!\n");
...
> 最后在对应的内核config中开启CONFIG_USB_NET_SR9800=y 即可
